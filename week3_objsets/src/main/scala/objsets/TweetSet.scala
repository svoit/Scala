...
abstract class TweetSet {
  ...
  /**
    * This method checks if `TweetSet` is empty.
    */
  def isEmpty: Boolean
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def loop(lc: TweetSet, rc: TweetSet): Tweet = {
      if(lc.isEmpty && rc.isEmpty) elem
      else if(lc.isEmpty) rc.mostRetweeted
      else lc.mostRetweeted
    }
    loop(left.filter(tw => tw.retweets > elem.retweets), right.filter(tw => tw.retweets > elem.retweets))
  }
